...
const int d_constraint_min[] { -50, 30, 60}; // ## {deg, deg, deg}/*最小角度限制單位deg*/
const int d_constraint_max[] {80, 150, 140}; // ## {deg, deg, deg}/*最大角度限制單位deg*/
...
uint16_t packetSize; /*預期DMP封包大小(預設42bytes)*/
...
void set_joint(int leg, int joint, float deg) {
  int _min = s_offset_min[leg][joint];/*腿部關節最小補償值*/
  int _max = s_offset_max[leg][joint];/*腿部關節最大補償值*/
  ...
  if (deg < _minC)/*若最小值大於角度*/
    deg = _minC;/*最小值assign角度*/
  else if (deg > _maxC)/*若最大值小於角度*/
    deg = _maxC;/*最大值assign角度*/
...
void set_servo(int leg, int joint, float pulse) {/*設定servo*/
