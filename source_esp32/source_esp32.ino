...
const float frequency = 440.0; // ## Hz  /*設定頻率參數單位赫茲*/

/// Kinematics Parameters

//: stores the location, rotation and scale of the main [body]
const datatypes::Transform body_transform = {
  {0, 0, 0},  // ## {mm, mm, mm} /*設定主體的位置三維參數單位mm*/
  {0, 0, 0},   // ## {deg, deg, deg} /*設定主體的旋轉參數單位deg*/
  {300, 40, 180} // ## {mm, mm, mm} /*設定主體的比例三維參數單位mm*/
};
...
const float bone_length = 105; // ## mm /*設定骨骼長度參數單位mm*/
...
const float precision = 0.001; // ## mm /*設定高等參數單位mm*/

void setup() {
  Serial.begin(115200);

  init_hardware();/*引入init_hardware()函數*/
  init_input();/*引入init_input()函數*/
}

//: those local variables control the step direction and period
datatypes::Vector2D _direction = {0, 0};
float turn = 0; //> /*指示旋轉方向*/
float height = 0; //> /*指示腿部伸展*/

int state = 0; //> /*指示步伐的型態(0) 閒置, (1) 小跑步, (2) 橫擺, (3) 俯仰滾動, (4) 物體偵測*/
float _period = 10.0; //> /*指示每秒步伐的數量*/

datatypes::Rotator _sRotation; //> /*這個變數儲存身體的相對旋轉*/

unsigned long duration;/*設定持續時間變數*/
int sample_sum, sample_num = 10,/*設定樣本和與樣本數量*/
                sample_index;/*設定樣本索引*/
float freq;/*設定頻率*/

void loop() {
  duration = millis();/*將內建時間函數millis()的值assign給duration*/

  handle_hardware();/*呼叫handle_hardware()函式*/
  handle_kinematics(_direction, turn, height, _period);/*呼叫handle_kinematics()並引入持續時間、選轉方向、高度、週期參數到函式裡*/

  handle_input();/*呼叫handle_input()函式*/

  if (Serial.available())/*判斷串口有無數據進來*/
    handle_serial();/*呼叫handle_serial()函式*/

  /*這個程式碼獲得loop function的頻率*/
...
void handle_serial() {
  ...
  while (Serial.available()) {
    char c = Serial.read(); /*將序列阜讀到的數值assign到c*/
    if (c == 13 || c == 32 || c == '\n') {
...
void commands_exe(float val1, float val2, float val3) {/*校正函式*/
  ...
  //: properties 1 is used for small adjustments to balance the weight
    ...
    base_offset[leg] = ammount;/*ammount assign 到 base_offset*/
  }
}
