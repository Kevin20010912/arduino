...
const float l_inv[4][2] = {/*儲存每個parent joint的反方向*/
  { +1.f, -1.f}, // ## {dir, dir}
  { -1.f, -1.f}, // ## {dir, dir}
  { -1.f, +1.f}, // ## {dir, dir}
  { +1.f, +1.f}  // ## {dir, dir}
...
float stored_0x = 0.f;
void handle_kinematics(datatypes::Vector2D _dir, float _turn, float _height, float period) {/*運動學方程式*/
  for (int l = 0; l < 4; l++) {
...
float c_base(float angle1) {/*base計算方程式*/
...
datatypes::Vector yaw_axis(int leg, float yaw) {/*yawl軸方程式*/
...
datatypes::Vector2D c_direction_ratio(datatypes::Vector2D dir) {/*方向比例計算方程式*/
...
datatypes::Rotator k_model(float x0, float y0, float z0,/*反方向運動學方程式*/
...
datatypes::Rotator c_triangle(float a0, float a1, float b0) {/*最後三角計算方程式*/
  float angle1 = a1;
